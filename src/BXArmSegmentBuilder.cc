//See BXArmSegmentBuilder.hh for more details
...
BXArmSegmentBuilder::BXArmSegmentBuilder()
{
...
// Construction of one Blowfish arm.
// We cannot build the cell and arm pieces seperatly anymore
// since now a more complete desctription of a cell includes
// pieces that fit through the hole in the arm.
// Since geant cannot allow overlapping volumes (even container
// volumes) we must build all the pieces together in one container.
//
// So the strategy is to build up an arm segment and put them
// together to make an arm
// Thefore building the cell is imbedded in building the arm segment.

G4int BXArmSegmentBuilder::Build(G4VSensitiveDetector * sd, G4bool showContainer,G4bool useWireframe)
{
	//if the builder has already been sucessfully run
	if( theArmSegLogicalVolume != 0) return 1;
	...
	if(Aluminum == 0 || BC505 == 0 || Lucite == 0 || Poly == 0 || Air == 0 || Silicone == 0 || Glass == 0){
		G4cerr << "BXCellBuilder::Build() -> ERROR: Cannot find materials needed to build the cell." << G4endl;
		return 2; }
	...
	if(useWireframe) { armAtt->SetForceWireframe(true); }
	else { armAtt->SetForceSolid(true); }
	...
	if(useWireframe) {wrapAtt->SetForceWireframe(true);}
    	else {wrapAtt->SetForceSolid(true);}
	...
	if(useWireframe) {luciteAtt->SetForceWireframe(true);}
    	else {luciteAtt->SetForceSolid(true);}
	...
	if(useWireframe) {housingAtt->SetForceWireframe(true);}
    	else {housingAtt->SetForceSolid(true);}
